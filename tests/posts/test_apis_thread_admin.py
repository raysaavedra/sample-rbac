...
def test_posts_list_success_for_thread_admin(api_client):
...
@pytest.mark.django_db
def test_posts_detail_success_for_thread_admin(api_client):
    _setup_user(api_client.user)
...
@pytest.mark.django_db
def test_posts_create_success_for_thread_admin(api_client):
    _setup_user(api_client.user)
    url = reverse('posts-list')
...
@pytest.mark.django_db
def test_posts_update_success_for_thread_admin(api_client):
    _setup_user(api_client.user)
    topic = baker.make(Topic)
    board = baker.make(Board, topic=topic)
    thread = baker.make(Thread, board=board)
...
@pytest.mark.django_db
def test_posts_delete_success_for_thread_admin(api_client):
    _setup_user(api_client.user)
    topic = baker.make(Topic)
    board = baker.make(Board, topic=topic)
    thread = baker.make(Thread, board=board)
